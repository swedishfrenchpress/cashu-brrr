<script lang="ts">
  ...
  const connect = async () => {
    try {
      isConnecting = true;
      const cleanUrl = mintUrl.replace(/^https?:\/\//, '');
...
<div class="flex flex-col gap-8 justify-center items-center w-full">
    ...
    <div class="flex-1">
      <div class="flex">
        ...
        <input
          ...
          bind:value={mintUrl}
          class="rounded-none rounded-e-lg bg-transparent text-[#0E0F0C] text-sm 
                 border border-[#868685] block flex-1 min-w-0 w-full p-3
  ...
  <div class="flex flex-wrap gap-2 justify-start w-full">
    ...
    <button
      class="inline-flex items-center justify-center px-4 py-2 font-medium
             text-[#454745] bg-slate-50/80 rounded-lg
             hover:bg-slate-100 transition-all duration-200
             border border-dashed border-[#868685]
             shadow-[0_1px_2px_0_rgba(0,0,0,0.05)]"
